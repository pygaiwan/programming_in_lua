...
function transliterate(s, t)
    local modified_s = s
    for k in pairs(t) do
        if t[k] ~=false then modified_s = modified_s:gsub(k, t[k])
        else modified_s = modified_s:gsub(k, '') end
    end
    return modified_s
end
...
describe ("Replace each char. All found.\n", function ()
    s = transliterate(original, {['a'] = 'b', ['l'] = 'd', ['e'] = 'd'})
...
describe ("Remove all chars. Result in empty string.\n", function ()
    s = transliterate('aaaa', {['a'] = false, ['l'] = false, ['e'] = false})
    it('Check all chars removed', function () assert.is.equal(s, '') end)
end)

describe ("Original string not modified\n", function ()
    s = transliterate(original, {['a'] = 'b', ['l'] = 'd', ['e'] = 'd'})
    it('Check original is same as before', function () assert.is.equal(original, 'la bella lava il fosso.') end)
end)
